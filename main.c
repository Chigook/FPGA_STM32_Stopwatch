...
int main(void)
{
  ...
  Button_GetInstance(&runstopButton, GPIOC, GPIO_PIN_13);

  Button resetButton;
  Button_GetInstance(&resetButton, GPIOB, GPIO_PIN_13);

  Led runstopLed;
  Led_GetInstance(&runstopLed, GPIOA, GPIO_PIN_5);
  /* USER CODE END 2 */
  ...
  int number = 0;
#define stopwatch_stop 0
#define stopwatch_run 1
  int stopwatch_state = stopwatch_stop;
  int prevNumber = 0;
  while (1)
  {
	  ...
	  case stopwatch_stop :
		  ...
		  if(Button_GetState(&runstopButton) == ACTIVE_RELEASE){
			  Led_On(&runstopLed);
